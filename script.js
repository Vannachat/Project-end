...
// --- PRESET SCENES (จำลองฉาก) ---

// 1. จำลองรั้วบ้าน
document.getElementById('btn-preset-fence')?.addEventListener('click', () => {
    clearAllObjects();

    const wallMat = new THREE.MeshLambertMaterial({ color: 0x8e44ad });
    const colMat = new THREE.MeshLambertMaterial({ color: 0xe67e22 });

    // รั้วแนวนอน 2 แผง + เสา 3 ต้น
    const positions = [
        { type: 'col', pos: [-4, 1, 0], name: 'Fence Column 1' },
        { type: 'wall', pos: [-2, 0.9, 0], name: 'Fence Wall 1', size: [3.6, 1.8, 0.15] },
        { type: 'col', pos: [0, 1, 0], name: 'Fence Column 2' },
        { type: 'wall', pos: [2, 0.9, 0], name: 'Fence Wall 2', size: [3.6, 1.8, 0.15] },
        { type: 'col', pos: [4, 1, 0], name: 'Fence Column 3' }
    ];

    positions.forEach(item => {
        let mesh;
        if (item.type === 'col') {
            mesh = new THREE.Mesh(new THREE.BoxGeometry(0.4, 2, 0.4), colMat.clone());
        } else {
            mesh = new THREE.Mesh(new THREE.BoxGeometry(...item.size), wallMat.clone());
        }
        mesh.position.set(...item.pos);
        addMesh(mesh, item.name);
    });
...
// 2. จำลองโรงรถ
document.getElementById('btn-preset-garage')?.addEventListener('click', () => {
    clearAllObjects();

    const slabMat = new THREE.MeshLambertMaterial({ color: 0x3498db });
    const colMat = new THREE.MeshLambertMaterial({ color: 0xe67e22 });

    // พื้นโรงรถ 5x5 เมตร
    const slab = new THREE.Mesh(new THREE.BoxGeometry(5, 0.2, 5), slabMat);
    slab.position.set(0, 0.1, 0);
    addMesh(slab, "Garage Slab");
    ...
    colPositions.forEach((pos, idx) => {
        const col = new THREE.Mesh(new THREE.BoxGeometry(0.3, 3, 0.3), colMat.clone());
        col.position.set(...pos);
        addMesh(col, `Garage Column ${idx + 1}`);
    });
